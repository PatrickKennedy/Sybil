...
class MemcachedCount(object):
    ...
    def __init__(self, name):
        self.key = 'MemcachedCount:%s' % name
...
class Counter(object):
    ...
    def __init__(self, name, shards=5):
        self.name = name
        self.memcached = MemcachedCount('Counter:%s' % name)
        self.delayed_incr = MemcachedCount('DelayedIncr:%s' % name)
        self.shards = shards

    def delete(self):
        del self.memcached.count
        del self.delayed_incr.count
        q = db.Query(CounterShard).filter('name =', self.name)
        shards = q.fetch(limit=self.shards)
        db.delete(shards)
    ...
    def get_count_and_cache(self):
        ...
        count = datastore_count + (self.delayed_incr.count or 0)
    ...
    count = property(get_count, set_count, delete)
...
class CounterShard(db.Model):
    ...
    @classmethod
    def increment(cls, counter, incr=1, shards=None):
        index = random.randint(1, counter.shards)
        counter_name = counter.name
        delayed_incr = (counter.delayed_incr.count or 0)
        shard_key_name = 'Shard:%s:%d' % (counter_name, index)
